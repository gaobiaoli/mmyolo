...
@MODELS.register_module()
class YOLOv8DCNCSPDarknet(YOLOv8CSPDarknet):
    arch_settings = {
        "P5": [
            [64, 128, 3, True, False],
            [128, 256, 6, True, False],
    ...
    def __init__(
        self,
        arch: str = "P5",
        stage_with_dcn: List[bool] = [False, True, True, True],
        last_stage_out_channels: int = 1024,
        plugins: Union[dict, List[dict]] = None,
        deepen_factor: float = 1.0,
        widen_factor: float = 1.0,
        ...
        self.stage_with_ak = stage_with_dcn
        super().__init__(
            arch=arch,
            last_stage_out_channels=last_stage_out_channels,
            deepen_factor=deepen_factor,
...
class AKConvModule(nn.Module):
    def __init__(self, inc, outc, kernel_size, stride=1, bias=None):
        ...
        self.p_conv = nn.Conv2d(
            inc, 2 * kernel_size, kernel_size=3, padding=1, stride=stride
        )
