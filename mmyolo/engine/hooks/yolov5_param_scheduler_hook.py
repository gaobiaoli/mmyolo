...
def mix_fn(lr_factor: float, max_epochs: int):
    T = max_epochs // 2
    """Generate mix function."""
    return (
        lambda x: ((1 - math.cos((x-T) * math.pi / T)) / 2) * (lr_factor - 1) + 1
        if x > T
        else 1.0
    )
...
class YOLOv5ParamSchedulerHook(ParamSchedulerHook):
    ...
    scheduler_maps = {"linear": linear_fn, "cosine": cosine_fn, "mix": mix_fn}
